...
### 与同类项目的关系（RepoBrain / claude-mem）

常被拿来比较的两个同类项目：[RepoBrain](https://github.com/study8677/repobrain)、[claude-mem](https://github.com/thedotmack/claude-mem)。三者补的都是「聊天窗口不够」，但**不在同一层——可互补，不宜互相替代**：

| 项目 | 一句话 | 事实源 |
|------|--------|--------|
| **RepoBrain** | 让 AI 读懂代码库（问答） | 源码 |
| **claude-mem** | 让 AI 记住跨会话做过什么（自动记事） | 会话轨迹 |
| **本套件** | 让 AI 真能动手改，并**改完能自证** | 代码 + 运行态 |

- **RepoBrain 管「怎么实现的？」**、**claude-mem 管「上次聊到哪」**、**本套件管「这次改得对不对」**——查库 / 自检 / 冒烟 / 对账 + 进度 SSOT。
- **本套件不建代码库问答索引，也不自动记忆会话。** 日常靠 SSOT 落点 + `rg` + `scan-project` 热文件摸到现行逻辑，只读命中文件；改完用 `lint` / `smoke` / `api-check`（默认摘要）证明没猜错。填 `AGENTS.md`、按技术栈适配 `scripts/`（约 30–60 分钟）——**分水岭不是「谁写的记忆」，而是「记忆能不能被代码复核」。**
- 三者可并存，但**进度只认本套件的 SSOT**；与实测结果冲突时，以实测为准。
- 想用大仓问答 / 自动会话记忆，另装那两个项目即可；**本套件不把它们作为依赖**。
